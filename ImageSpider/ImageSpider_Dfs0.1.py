...
def get_levels():
    global levels
    while True:
        levels = input(r"设定爬取层数:")
        if levels is None:
            return
        reg = r'^\d+$'
        pattern = re.compile(reg)
        regret = re.match(pattern, levels, flags=0)
        if regret is None:
            print("爬取层数无效，请重新输入")
            continue
        return
    ...
    if len(imgs) > 0:
        ...
        for img in imgs:
            ...
            try:
                ...
                request.urlretrieve(img_url, local_dir % n)
                #已下载过的图片链接记录
...
def dfs(pages):
    ...
    if levels is not None or levels != "":
        if levels_count > int(levels):
            levels_count = 0
            return
