...
def main():
    while True:
        image_file = input(r"请输入图片地址:")
        regx = r'^[a-zA-Z]:(((\\(?! )[^/:*?<>\""|\\]+)+\\?)|(\\)?)\.jpg$'
        regx_ret = re.match(regx, image_file)
        if regx_ret is None:
            print(r'路径无效，请重新输入')
            continue
        break
    try:
        ...
        print(r'图片打开失败')
        return
    print('Info:',end="");print(image_file.size[0],end="");print(image_file.size[1],end="");print(count)
    try:
        tmp = open(r'.\imgCd_result.txt','w')
